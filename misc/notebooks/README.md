...
* 22_BayesianQuantumAmplitudeEstimation.ipynb: This notebook presents the Bayesian approach for amplitude estimation.

* 23_qml4var_unsupervised_training_dft.ipynb: This notebook introduces an unsupervised qml4var training routine using empirical CDF labels from samples, feature re-scaling from `[-2π, 2π]` to `[-π, π]`, and DFT coefficient extraction from direct circuit evaluations in `[-2π, 2π]`.

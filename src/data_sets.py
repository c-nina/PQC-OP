...
def create_random_data(**kwargs: Any):
    ...
    test_x = np.array(list(product(*[test_x[:, i] for i in range(test_x.shape[1])])))
    print(test_x.shape)
    # Create Labels
    ...
    if feature_number == 1:
        ...
        covs_ = [[int(i == j) for j in range(feature_number)] for i in range(feature_number)]
...
def create_bs_data(**kwargs: Any):
    ...
    # Create Labels
    train_y = empirical_cdf(np.reshape(train_x, (n_points_train, feature_number))) - 0.5
